...
//	FB::log($results);
	...
	if(!empty($results)) {
		foreach($results as $k=>$v) {
            ...
            <section class="search-result-artist">
             <div class="artist-search-result">
              <span class="img">
                <a href="#">
                  <img src ="'.$artist['image']['large'].' " >
                </a>
              </span>
            </a>
              <span class="text">
                <p class="name">'.$_POST['artist'].'</p>
                <p class="button">
                   <span onclick="" href="#" class="register-button"  id="register" title="'.$_POST['artist'].'" name="'. $artist['name'].'" value="'.$artist['image']['large'].'">
                  </span>
                </p>
              </span>
          </div>
		...
		$tracks_html .= '<tr><td colspan="3">アーティストが見つかりませんでした</td></tr>';
	}
...
// after a Track is requested reload requested tracks so that it 